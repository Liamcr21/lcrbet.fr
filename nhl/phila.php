...
<h2 class=sous-titre>Philadelphia Flyers</h2>
<img src="../img/nhl/philadelphia-flyers-logo.png" alt="logo" class="img-ekip" >
...
try {
    ...
    foreach($dbh->query(' SELECT * FROM `nhl_stats` WHERE ID IN (22); `') as $row) {
...
try {
    $dbh = new PDO('mysql:host=localhost;dbname=u801021231_BDDNHLlcrbet', $user, $pass);
    foreach($dbh->query(' SELECT LOGO,PLAYER,GOAL FROM `buteur_nhl` WHERE ID IN (23) ORDER BY GOAL DESC LIMIT 10; `') as $row) {
        $row = array_map("utf8_encode", $row);
